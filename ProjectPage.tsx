...
const ProjectPage: React.FC<{ projects: Project[] }> = ({ projects }) => {
  ...
  const addComponent = () => {
    if (!newComponent.type || !newComponent.pin || !newComponent.mode) {
      setError("Please fill out all fields to add a component.");
      return;
    }
    setError(null);
  ...
  const generateCode = () => {
    let code = `// Auto-generated code for project: ${project?.name}\n\n`;
    code += `void setup() {\n`;

    // Generate setup code for each component
    ...
    // Add behavior for each component
    components.forEach((component) => {
      if (component.type === "LED" && component.mode === "Output") {
        code += `  digitalWrite(${component.pin}, HIGH); // Turn LED on\n`;
        code += `  delay(1000); // Wait 1 second\n`;
        code += `  digitalWrite(${component.pin}, LOW); // Turn LED off\n`;
        code += `  delay(1000); // Wait 1 second\n`;
      }
    });
        ...
        <>
          <div className="component-form">
            ...
            <label>
              ...
              <select
                ...
                onChange={(e) => setNewComponent({ ...newComponent, pin: e.target.value })}
              >
            ...
            <label>
              ...
              <select
                ...
                onChange={(e) => setNewComponent({ ...newComponent, mode: e.target.value })}
              >
            ...
            <div className="components-list">
              ...
              <ul>
                {components.map((component, index) => (
                  <li key={index}>
                    {component.type} (Pin: {component.pin}, Mode: {component.mode})
          ...
          {generatedCode && (
            <div className="generated-code">
              ...
              <textarea rows={20} cols={80} value={generatedCode} readOnly />
            </div>
          )}
        </>
      )}
